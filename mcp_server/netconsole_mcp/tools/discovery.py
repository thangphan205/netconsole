...
@mcp.tool()
async def discover_switches(cidr: str, port: int = 22, tcp_timeout: float = 1.0) -> dict:
    """
    ...
    existing_switch_id, existing_hostname}]}. IPv4 only; subnets larger than
    /22 (>1024 hosts) are rejected. This only TCP-probes the port — it does not
    log in. Follow with identify_discovered_switches on the open, non-existing
    IPs. Superuser API key required.
    """
    return await client.post(
        "/switches/discovery/scan",
...
@mcp.tool()
async def identify_discovered_switches(
...
@mcp.tool()
async def bulk_add_discovered_switches(switches: list[dict]) -> dict:
    """
    Bulk-add reviewed discovery candidates as switches. Each dict is a switch:
