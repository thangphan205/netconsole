from fastapi.testclient import TestClient

from app.core.config import settings
from app.tests.utils.utils import random_lower_string


def test_read_devices(
    client: TestClient, superuser_token_headers: dict[str, str]
) -> None:
    r = client.get(f"{settings.API_V1_STR}/devices/", headers=superuser_token_headers)
    assert r.status_code == 200
    data = r.json()
    assert "data" in data
    assert "count" in data


def test_create_device(
    client: TestClient, superuser_token_headers: dict[str, str]
) -> None:
    hostname = f"testdevice_{random_lower_string()[:8]}"
    payload = {"hostname": hostname, "ipaddress": "10.0.0.1"}
    r = client.post(
        f"{settings.API_V1_STR}/devices/",
        headers=superuser_token_headers,
        json=payload,
    )
    assert r.status_code == 200
    sw = r.json()
    assert sw["hostname"] == hostname
    assert sw["ipaddress"] == "10.0.0.1"
    assert "id" in sw
    # cleanup
    client.delete(
        f"{settings.API_V1_STR}/devices/{sw['id']}",
        headers=superuser_token_headers,
    )


def test_create_device_duplicate_hostname(
    client: TestClient, superuser_token_headers: dict[str, str]
) -> None:
    hostname = f"dupdevice_{random_lower_string()[:8]}"
    payload = {"hostname": hostname, "ipaddress": "10.0.0.2"}
    r1 = client.post(
        f"{settings.API_V1_STR}/devices/",
        headers=superuser_token_headers,
        json=payload,
    )
    assert r1.status_code == 200
    sw_id = r1.json()["id"]
    r2 = client.post(
        f"{settings.API_V1_STR}/devices/",
        headers=superuser_token_headers,
        json=payload,
    )
    assert r2.status_code == 400
    # cleanup
    client.delete(
        f"{settings.API_V1_STR}/devices/{sw_id}",
        headers=superuser_token_headers,
    )


def test_read_device(
    client: TestClient, superuser_token_headers: dict[str, str]
) -> None:
    hostname = f"getsw_{random_lower_string()[:8]}"
    r = client.post(
        f"{settings.API_V1_STR}/devices/",
        headers=superuser_token_headers,
        json={"hostname": hostname, "ipaddress": "10.0.0.3"},
    )
    sw_id = r.json()["id"]
    r2 = client.get(
        f"{settings.API_V1_STR}/devices/{sw_id}",
        headers=superuser_token_headers,
    )
    assert r2.status_code == 200
    assert r2.json()["id"] == sw_id
    client.delete(
        f"{settings.API_V1_STR}/devices/{sw_id}",
        headers=superuser_token_headers,
    )


def test_read_device_not_found(
    client: TestClient, superuser_token_headers: dict[str, str]
) -> None:
    r = client.get(
        f"{settings.API_V1_STR}/devices/999999",
        headers=superuser_token_headers,
    )
    assert r.status_code == 404


def test_update_device(
    client: TestClient, superuser_token_headers: dict[str, str]
) -> None:
    hostname = f"updsw_{random_lower_string()[:8]}"
    r = client.post(
        f"{settings.API_V1_STR}/devices/",
        headers=superuser_token_headers,
        json={"hostname": hostname, "ipaddress": "10.0.0.4"},
    )
    sw_id = r.json()["id"]
    r2 = client.put(
        f"{settings.API_V1_STR}/devices/{sw_id}",
        headers=superuser_token_headers,
        json={"ipaddress": "10.0.0.99"},
    )
    assert r2.status_code == 200
    assert r2.json()["ipaddress"] == "10.0.0.99"
    client.delete(
        f"{settings.API_V1_STR}/devices/{sw_id}",
        headers=superuser_token_headers,
    )


def test_delete_device(
    client: TestClient, superuser_token_headers: dict[str, str]
) -> None:
    hostname = f"delsw_{random_lower_string()[:8]}"
    r = client.post(
        f"{settings.API_V1_STR}/devices/",
        headers=superuser_token_headers,
        json={"hostname": hostname, "ipaddress": "10.0.0.5"},
    )
    sw_id = r.json()["id"]
    r2 = client.delete(
        f"{settings.API_V1_STR}/devices/{sw_id}",
        headers=superuser_token_headers,
    )
    assert r2.status_code == 200
    r3 = client.get(
        f"{settings.API_V1_STR}/devices/{sw_id}",
        headers=superuser_token_headers,
    )
    assert r3.status_code == 404


def test_device_requires_auth(client: TestClient) -> None:
    r = client.get(f"{settings.API_V1_STR}/devices/")
    assert r.status_code == 401


def test_update_device_metadata_auth_error(
    client: TestClient, superuser_token_headers: dict[str, str]
) -> None:
    from unittest.mock import patch

    from app.automation.devices import DeviceAuthenticationError

    hostname = f"mocksw_{random_lower_string()[:8]}"
    r = client.post(
        f"{settings.API_V1_STR}/devices/",
        headers=superuser_token_headers,
        json={"hostname": hostname, "ipaddress": "10.0.0.6"},
    )
    sw_id = r.json()["id"]

    with patch("app.crud.devices.get_metadata") as mock_get_metadata:
        mock_get_metadata.side_effect = DeviceAuthenticationError("Wrong password test")
        r2 = client.put(
            f"{settings.API_V1_STR}/devices/{sw_id}/metadata",
            headers=superuser_token_headers,
        )
        assert r2.status_code == 400
        assert "Authentication failed" in r2.json()["detail"]

    # Check that database has status AUTH_ERROR
    r3 = client.get(
        f"{settings.API_V1_STR}/devices/{sw_id}",
        headers=superuser_token_headers,
    )
    assert r3.json()["health_status"] == "AUTH_ERROR"

    client.delete(
        f"{settings.API_V1_STR}/devices/{sw_id}",
        headers=superuser_token_headers,
    )
